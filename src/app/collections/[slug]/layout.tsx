...
async function CollectionsLayout({ children, params: { slug } }: LayoutProps) {
  ...
  return <main className="mx-auto max-w-7xl py-10 px-5 space-y-10">
    <div className="flex flex-col gap-10">
        {banner && (
            <div className="relative hidden sm:block">
                ...
                <h1 className="bottom-10 left-1/2 -translate-x-1/2 absolute font-bold text-4xl text-white lg:text-5xl">
                    {collection.name}
                </h1>
            </div>
        )}
        <h1 className={cn("mx-auto text-3xl font-bold md:text-4xl", 
            banner && "sm:hidden"
        )}>
            {collection?.name}
        </h1>
