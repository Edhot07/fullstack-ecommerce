...
export default function ProductReviews({product}: ProductReviewsProps) {
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useInfiniteQuery({
    ...
    //The reviews was showing to itself without approval from the backend so resolve it using below
    select: (data)=>({
      ...data,
      pages: data.pages.map((page)=>({
        ...page,
        items: page.items.filter(
          (item) => item.moderation?.moderationStatus === reviews.ModerationModerationStatus.APPROVED,
        )
      }))
    }),
    ...
    {reply?.message && (
      <div className="ms-10 mt-2.5 space-y-1 border-t pt-2.5">
        <div className="flex items-center gap-2">
          ...
          className="size-5"
          />
          <span className="font-bold">EStore team</span>
        </div>
...
function MediaAttachment({media}: MediaAttachmentProps){
  ...
  //didn't use media.video because it's not full video url intead imported media from wix/sdk
  if(media.video){
    return <video className="max-h-40 max-w-40" controls>
