...
export default function Order({order}: OrderProps){
    ...
    const fullfilmentStatusMap: Record<orders.FulfillmentStatus, string> = {
    ...
    ? fullfilmentStatusMap[order.fulfillmentStatus]
    : null;
            ...
            <div className="basis-96">
                <div className="space-y-0 5">
                    <div className="flex items-center gap-3 font-semibold">
                        <span>Subtotal: {order.priceSummary?.subtotal?.formattedAmount}</span>
                        <Badge
            ...
            </div>
            {
                shippingDestination && (
                    <div className="space-y-0 5">
                        ...
                        <p>
                            {shippingDestination.address?.subdivision || shippingDestination.address?.country}
                        </p>
...
function OrderItem({item}: OrderItemProps){
    ...
    return <div className="flex flex-wrap gap-3 py-5 last:pb-0">
        <WixImage
        ...
        <div className="space-y-0 5">
            ...
            {!!item.descriptionLines?.length &&(
                <p>
                    {item.descriptionLines.map(line=> line.colorInfo?.translated || line.plainText?.translated,).join(", ")}
                </p>
            )}
        </div>
    </div>
